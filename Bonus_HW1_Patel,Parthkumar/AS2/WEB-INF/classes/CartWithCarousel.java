				...
				for (int j = i; j < (i + 3); j++) {
					...
					if (j < count) {
						_Content += ""
								...
								+ "<tr>"
								+ "<td>"
								+ "<form action='/AS2/AddToCart' name='requestedForm' value='AddToCart' method='post'>"
  ...
  private String content_footer(){
	...
	private String printCart() {
		String _Header = "<div id='body'>"
				+ "<section id='content'>"
				+ (userLoggedIn ? "<form action='/AS2/PlaceOrderServlet' method='post'>":"<form action='/AS2/LoginServlet' method='post'>")
				+ "<div class='box'>"
		...
		for (Map.Entry<Product, Integer> entry : cart.entrySet()) {
			...
			_Content +="<tr>"
					...
					+ "<td><a href='/AS2/RemoveFromCart?requestedForm=RemoveFromCart&&cartProduct="+p.getProductId()+"' class='btn btn-outline-dark' role='button'>Remove</a></td>";
			count++;
