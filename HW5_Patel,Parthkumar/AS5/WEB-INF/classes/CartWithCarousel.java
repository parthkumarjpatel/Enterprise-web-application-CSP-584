				...
				for (int j = i; j < (i + 3); j++) {
					...
					if (j < count) {
						_Content += ""
								...
								+ "</tr>"
								+ "<tr>" + "<td>"
								+ "<a class='btn btn-outline-dark' role='button' href='/AS5/ViewProductServlet?productId="
								+ accessory.get(j).getProductId() + "'>View Product</a>" 
								+ "</td>" + "</tr>" 
								+ "</table>"
								+ "</div>"
						;
  ...
  private String content_footer(){
	...
	private String printCart() {
		String _Header = "<div id='body'>"
				+ "<section id='content'>"
				+ (userLoggedIn ? "<form action='/AS5/PlaceOrderServlet' method='post'>":"<form action='/AS5/LoginServlet' method='post'>")
				+ "<div class='box'>"
		...
		for (Map.Entry<Product, Integer> entry : cart.entrySet()) {
			...
			_Content +="<tr>"
					...
					+ "<td><a href='/AS5/RemoveFromCart?requestedForm=RemoveFromCart&&cartProduct="+p.getProductId()+"' class='btn btn-outline-dark' role='button'>Remove</a></td>";
			count++;
