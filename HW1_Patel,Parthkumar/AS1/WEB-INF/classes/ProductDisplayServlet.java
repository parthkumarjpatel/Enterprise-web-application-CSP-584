		...
		for (Map.Entry<String, ArrayList<Product>> entry : product.entrySet()) {
			...
			for (Product p : prod) {
				_Product += "<div class='row row-space'>" + "<div class='col-xs-3 img-block'>" + ""
						...
						+ "<tr>"
						+ "<td>"
						+ "<a class='btn btn-link' role='button' href='/AS1/ViewProductServlet?productId="+p.getProductId()+"'>View Product</a>"
						+ "</td>"
						+ "</tr>"
						+ "</table>"
						+ "</form>"
		...
		for (Map.Entry<String, ArrayList<Product>> entry : product.entrySet()) {
			...
			for (Product p : prod) {
				_Product += "<div class='row row-space'>"
								...
								+ ""
								+ "</div>"
								+ "<div class='col-xs-6 divide-block'>" + "<h4 class=''>" + p.getModel()
								+ "</a>"
						 		+ "</h4>" + "<label class='control-label'>Manufacturer: </label>"
								+" <label class='info-label '>" + p.getManufacturer() + "</label>"
								+ "<x class='space'></x> |"
								+ "<label class='control-label space'>Condition: </label>"
								+" <label class='info-label '>" + p.getCondition() + "</label>"
								...
								+ "<label class='control-label space'>Discount: </label>"
								+" <label class='info-label'>" + p.getDiscount() + "% " + "</label> <br>"
								+ "<label " + (p.getQuantity() > 0 ? "class='availibility'>Currently Available</label>" : "class='availibility'>Currently Unavailable")
								+ "</div>"

								+ "<div class='col-xs-1 space-price-block '>"
								+ "<h3 class='price space'>$"	+ p.getPrice() + "</h3>"
								+ "<form action='/AS1/AddToCart' name='requestedForm' value='AddToCart' method='get'>"
								+ "<table>"
								+ "<tr>"
								+ "<td>"
