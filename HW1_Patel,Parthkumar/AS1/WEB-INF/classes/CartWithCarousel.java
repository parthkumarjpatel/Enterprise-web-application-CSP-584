  ...
  private String getCarousel(String productId) {
		...
		for (Map.Entry<Integer, ArrayList<Accessory>> entry : accessories.entrySet()) {
			...
			for (int i = 0; i < count; i +=3) {
				...
				_Content +=""
						+(i == 0 ? "<div class='carousel slide'>" : "<div class='carousel-item'>")
  ...
  private String content_footer(){
	...
	private String printCart() {
		String _Header = "<div id='body'>"
				+ "<section id='content'>"
				+ (userLoggedIn ? "<form action='/AS1/PlaceOrderServlet' method='post'>":"<form action='/AS1/LoginServlet' method='post'>")
				+ "<div class='box'>"
				+ "<div class='box-header'>"
				+ "<h3 class='box-title'>Cart Details</h3>"
				+ "	<button class='btn btn-outline-dark pull-right' type='Submit'>Place Order</button>"
				+ "</div>"
		...
		for (Map.Entry<Product, Integer> entry : cart.entrySet()) {
			...
			_Content +="<tr>"
					...
					+ "<td><a href='/AS1/RemoveFromCart?requestedForm=RemoveFromCart&&cartProduct="+p.getProductId()+"' class='btn btn-outline-dark' role='button'>Remove</a></td>";
			count++;
